...
var aacencoder_utf8ToStr;

function aacencoder(aacencoder_opts) {

    aacencoder_utf8ToStr = UTF8ArrayToString;
    aacencoder_opts = aacencoder_opts || {};
    var aacencoder_return;
    var Module = {};

    function aacencoder_toU8(data) {
    ...
    Object.keys(aacencoder_opts).forEach(function(key) {
        if (key != "mounts" && key != "MEMFS") {
            Module[key] = aacencoder_opts[key];
        }
    });

    Module["preRun"] = function() {
        (aacencoder_opts["mounts"] || []).forEach(function(mount) {
        ...
        (aacencoder_opts["MEMFS"] || []).forEach(function(file) {
            if (file["name"].match(/\//)) {
                throw new Error("Bad file name");
            }
            var fd = FS.open(file["name"], "w+");
            var data = aacencoder_toU8(file["data"]);
    ...
    Module["postRun"] = function() {
        ...
        var outFiles = listFiles("/work").filter(function(file) {
            ...
            var data = aacencoder_toU8(file.contents);
            return {"name": file.name, "data": data};
        });
        aacencoder_return = {"MEMFS": outFiles};
    };

    /*EMSCRIPTENBODY*/

    return aacencoder_return;
}
